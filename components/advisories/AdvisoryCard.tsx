      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1.5 flex-wrap">
            ...
            <Badge tone="paper">{KIND_LABEL[advisory.kind]}</Badge>
            <span className="text-[10px] font-mono text-ink/40">@ {formatClock(advisory.timestamp)}</span>
          </div>
          <p className="font-bold text-sm md:text-base leading-snug">{advisory.headline}</p>
          <p className="text-xs text-ink/60 mt-1 leading-snug">{advisory.rationale}</p>
        </div>
        {advisory.quantity ? (
